...
from sentiment.sentimentAnalysis.stopWords import StopWords
...
class TextProcessing():
    ...
    @staticmethod
    def replaceSpecialChar(dataset, colName):
        regexIgnore = "[^a-zA-Z ]"
        regexInclude = "[,.]"
    ...
    def createToken(self, dataset, colName):
        ...
        sentimentTokenizer = RegexTokenizer(inputCol=colName, outputCol="SA_tokenized",
                                            toLowercase=True,
                                            pattern="\\W")
    ...
    def stopWordsRemover(self, dataset, colName):
        dataset = dataset.drop("SA_stopWords")
        stopWordsList = StopWords.stopWordsKNIME
        sentimentStopWordRemover = StopWordsRemover(inputCol=colName,
                                                    outputCol="SA_stopWords")  # .loadDefaultStopWords("english") #change the language based on user preference.
    ...
    def ngrams(dataset, colName, number):
        dataset = dataset.drop("SA_ngrams")
        ngram = NGram(n=number, inputCol=colName, outputCol="SA_ngrams")
        dataset = ngram.transform(dataset)
        return dataset

    def sparkLemmatizer(self, dataset, colName, lemmatizedModelPath):
        dataset = dataset.select("SA_index", concat_ws(",", dataset[colName]).alias(colName))
        ...
        dataset = dataset.withColumn("lemma_result", array_join("lemma.result", ""))
        sentimentTokenizer = RegexTokenizer(inputCol="lemma_result", outputCol="SA_lemma",
                                            toLowercase=True,
                                            pattern="\\W")
        dataset = sentimentTokenizer.transform(dataset)
